...
namespace Gree.AirConditioner
{
    static class VariableExtension
    {
        ...
        static public string SECRET_KEY { get; private set; }
        ...
        static VariableExtension()
        {
            foreach (DictionaryEntry arg in Environment.GetEnvironmentVariables())
            {
                ...
                if (arg.Key.ToString() == "SECRET_KEY")
                {
                    SECRET_KEY = arg.Value?.ToString();
                }
            ...
            {
                throw new ArgumentException("SECRET_KEY Environment variable is required!");
            }
        }
    }
}
