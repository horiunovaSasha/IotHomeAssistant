...
namespace Zigbee2Mqtt.Motion
{
    ...
    {
        private const string MQTT_CLIENT_ID = "Zigbee2Mqtt.Motion";
            ...
            _mqttClient.UseApplicationMessageReceivedHandler(async e =>
            {
                ...
                if (e.ApplicationMessage.Topic == $"zigbee2mqtt/{VariableExtension.DEVICE_ID}")
                {
                    ...
                    if (status != null)
                    {
                        _lastState = new
                        {
                            Event = status.Occupancy ? "motion_detected" : "motion_stopped",
                            Value = status.Occupancy ? "ЗАФІКСОВАНО РУХ" : "рух відсутній"
                        };
