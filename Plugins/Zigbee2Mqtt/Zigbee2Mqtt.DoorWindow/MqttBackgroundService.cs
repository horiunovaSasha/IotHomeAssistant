...
namespace Zigbee2Mqtt.DoorWindow
{
    ...
    {
        private const string MQTT_CLIENT_ID = "Zigbee2Mqtt.DoorWindow";
            ...
            _mqttClient.UseApplicationMessageReceivedHandler(async e =>
            {
                ...
                if (e.ApplicationMessage.Topic == $"zigbee2mqtt/{VariableExtension.DEVICE_ID}")
                {
                    ...
                    if (status != null)
                    {
                        _lastState = new
                        {
                            Event = status.Contact ? "doorwindow_opened" : "doorwindow_closed",
                            Value = status.Contact ? "Відкрито" : "Закрито"
                        };
